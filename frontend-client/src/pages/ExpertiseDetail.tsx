...
export default function ExpertiseDetail() {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="flex gap-1.5">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="w-2 h-2 rounded-full bg-[#00d4b8] animate-bounce"
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-2xl text-slate-900 mb-4">Expertise non trouvée</p>
          <Link to="/expertises" className="text-[#1e40af]">
            ← Retour
          </Link>
        </div>
      </div>
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            >
              <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#0d1435] to-[#1a2550] border border-white/10 p-8 min-h-[340px] flex flex-col justify-between">
                <div className="absolute inset-0 bg-gradient-to-br from-purple-500/10 to-blue-600/5" />
                <div className="relative">
                  <div className="flex items-center gap-3 mb-6">
                    <span className="text-4xl">{expertise.icon || "⚖️"}</span>
                    <div>
                      <p className="text-xs text-[#00d4b8] font-semibold uppercase tracking-wider">
                        Expertise
                      </p>
                ...
                <div className="relative grid grid-cols-3 gap-4 mt-8 pt-6 border-t border-white/10">
                  {[
                    ...
                    <div key={l}>
                      <p className="text-xl font-bold text-[#00d4b8]">{n}</p>
            ...
            >
              <p className="text-sm font-semibold text-[#00d4b8] uppercase tracking-wider mb-3">
                Notre expertise
              </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {expertise.legal_elements.map((el: string, i: number) => (
                      <div key={i} className="flex items-center gap-2.5">
                        <IconCircleCheck
                          size={14}
                          className="text-[#00d4b8] flex-shrink-0"
                        />
              ...
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 mt-8 text-white px-6 py-3.5 rounded-2xl font-semibold transition active:scale-95 hover:opacity-90"
      ...
      {team.length > 0 && (
        <section className="py-16 px-6 bg-[#080d1e]">
          <div className="max-w-6xl mx-auto">
            <p className="text-xs font-semibold text-[#00d4b8] uppercase tracking-widest mb-2">
              Equipe dediee
            </p>
              ...
              {team.map((m: any) => {
                ...
                return (
                  <Link
                    key={m.id}
                    to={"/equipe/" + m.id}
                    className="group flex items-center gap-4 glass-card px-5 py-4 hover:border-[#00d4b8]/40 transition min-w-[220px]"
                  >
                    <div className="w-12 h-12 rounded-full overflow-hidden shrink-0 ring-2 ring-slate-200 group-hover:ring-[#00d4b8]/40 transition">
                      {m.photo ? (
                        <img
                          src={
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-900 group-hover:text-[#00b8a0] transition">
                        {m.name}
                      </p>
                    ...
                    <IconArrowRight
                      size={14}
                      className="ml-auto text-slate-400 group-hover:text-[#00b8a0] transition shrink-0"
                    />
                  </Link>
                );
              })}
      ...
      {others.length > 0 && (
        <section className="py-16 bg-[#080d1e]">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <p className="text-xs font-semibold text-[#00d4b8] uppercase tracking-wider mb-2">
              Autres domaines
            </p>
            <h2 className="text-2xl font-bold text-white mb-8">
              ...
              {others.map((e: any, i: number) => (
                ...
                >
                  <Link
                    ...
                    className="glass-card p-4 flex flex-col gap-3 hover:border-[#00d4b8]/40 hover:-translate-y-1 transition-all group block"
                  >
                    <span className="text-2xl">{e.icon || "⚖️"}</span>
                    <p className="text-sm font-semibold text-slate-900 group-hover:text-[#00b8a0] transition leading-tight">
                      {e.name}
                    </p>
                    <p className="text-xs text-slate-500 line-clamp-2">
                      {e.summary}
                    </p>
                    <div className="flex items-center gap-1 text-[#00b8a0] text-xs font-medium mt-auto">
                      En savoir plus <IconArrowRight size={11} />
                    </div>
                  </Link>
                </motion.div>
              ))}
      ...
      {/* CTA final */}
      <section className="py-16 border-t border-white/10" style={{ background: "linear-gradient(135deg, #1a0e5c 0%, #2d1b6b 100%)" }}>
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <a
              href="tel:0491335000"
              className="inline-flex items-center gap-2 text-white px-6 py-3.5 rounded-2xl font-semibold transition active:scale-95 hover:opacity-90"
              style={{ background: "linear-gradient(90deg, #00d4b8 0%, #00b0f0 100%)" }}
            >
              <IconPhone size={16} />
              04 91 33 50 00
