...
import { IconSearch, IconPlus, IconMinus, IconChevronRight, IconHelpCircle } from '@tabler/icons-react';
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-[#080d1e] h-150 pt-10 pb-20">
        ...
        <div aria-hidden className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute bottom-0 left-0 w-[400px] aspect-square rounded-full bg-purple-600/10 blur-[120px]"/>
        </div>

        <div className="relative z-10 max-w-4xl mx-auto px-6 mt-25 text-center">
          <motion.div initial="hidden" animate="visible" variants={{ visible:{ transition:{ staggerChildren:0.1 } } }}>
            <motion.p variants={fadeUp} className="text-xs uppercase tracking-[0.3em] text-purple-400 mb-4 font-semibold">
              Centre d'aide
            </motion.p>
            ...
            <motion.div variants={fadeUp} className="relative max-w-xl mx-auto">
              ...
              <input
                ...
                className="w-full bg-white/8 border border-white/15 rounded-full pl-12 pr-4 py-3.5 text-white placeholder:text-white/35 outline-none focus:border-purple-500/40 transition text-sm"
              />
              {search && (
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              <IconHelpCircle size={40} className="text-white/20 mx-auto mb-4"/>
                  ...
                  <motion.div key={faq.id}
                    ...
                    >
                      ...
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition ${
                        isOpen ? 'bg-purple-500 text-white' : 'bg-white/8 text-white/50'
                      }`}>
                              ...
                              <div className="flex flex-wrap gap-1.5 mt-3">
                                {faq.keywords.split(',').filter(Boolean).map((k: string) => (
                                  <button key={k.trim()} onClick={() => setSearch(k.trim())}
                                    className="text-[11px] px-2 py-0.5 rounded-full bg-white/6 text-white/40 hover:bg-purple-500/15 hover:text-purple-400 transition">
                                    #{k.trim()}
                                  </button>
                                ))}
                              </div>
          ...
          <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
            ...
            <IconHelpCircle size={28} className="text-purple-400 mx-auto mb-3"/>
            ...
            <Link to="/contact"
              className="inline-flex items-center gap-2 text-white text-sm font-semibold px-6 py-2.5 rounded-full transition hover:opacity-90"
              style={{ background: "linear-gradient(90deg, #00d4b8 0%, #00b0f0 100%)" }}>
              Nous contacter <IconChevronRight size={15}/>
            </Link>
          </motion.div>
        </div>
      </section>
