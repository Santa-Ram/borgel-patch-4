      ...
      setNotifs([
        {
          id: 1,
          type: "contact",
          title: "Nouveau message de Marie D.",
          message: "Suite à mon accident...",
          is_read: false,
  ...
  const NavContent = () => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
        {navItems.map(({ icon: Icon, label, path }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive ? "bg-orange-500/15 text-orange-400 border border-orange-500/20" : "text-white/60 hover:text-white hover:bg-white/5"}`
            }
          >
      ...
      <div className="border-t border-white/8 p-3">
        <button
          onClick={logout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-red-400/70 hover:text-red-400 hover:bg-red-500/5 transition"
        >
          <LogOut size={14} />
          Déconnexion
        </button>
        ...
        <header className="flex items-center gap-3 px-4 h-14 bg-[#060b1a]/90 backdrop-blur-md border-b border-white/8 flex-shrink-0">
          ...
          <div className="flex-1 max-w-sm relative hidden sm:block">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/8 rounded-lg pl-8 pr-4 py-1.5 text-sm text-white placeholder:text-white/30 outline-none focus:border-orange-500/40 transition"
            />
          </div>
