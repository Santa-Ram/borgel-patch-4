...
import { MessageSquare, Users, Star, Mail, Video, Image, FileText, ArrowRight } from 'lucide-react';
...
const statusBadge: Record<string, JSX.Element> = {
  new:     <span className="badge badge-orange text-[10px]">Nouveau</span>,
  read:    <span className="badge text-[10px]" style={{ background: "#f1f5f9", color: "#64748b", border: "1px solid #e2e8f0" }}>Lu</span>,
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      API.get('/admin/videos/').then(r => r.data).catch(() => null),
    ]).then(([contacts_r, team_r, reviews_r, newsletter_r, posts_r, videos_r]) => {
      ...
      const v  = (videos_r?.results    || videos_r)    || [];
      ...
      if (c.length) {
        setStats(prev => ({
          ...
          videos:     { total: v.length,  active:    v.filter((x: any) => x.is_active).length },
        }));
  ...
  const cards = [
    { icon: MessageSquare, label: 'Messages', value: stats.contacts.total,   sub: `${stats.contacts.unread} non lus`,      color: 'text-orange-500', bg: 'bg-orange-50',  link: '/contacts'   },
    { icon: Users,         label: 'Membres',  value: stats.team.total,       sub: `${stats.team.active} actifs`,           color: 'text-blue-500',   bg: 'bg-blue-50',    link: '/team'       },
    { icon: Star,          label: 'Avis',     value: stats.reviews.total,    sub: `${stats.reviews.active} publiés`,       color: 'text-amber-500',  bg: 'bg-amber-50',   link: '/reviews'    },
    { icon: Mail,          label: 'Abonnés',  value: stats.newsletter.total, sub: `${stats.newsletter.active} actifs`,     color: 'text-green-500',  bg: 'bg-green-50',   link: '/newsletter' },
    { icon: FileText,      label: 'Posts',    value: stats.posts.total,      sub: `${stats.posts.published} publiés`,      color: 'text-violet-500', bg: 'bg-violet-50',  link: '/posts'      },
    { icon: Video,         label: 'Vidéos',   value: stats.videos.total,     sub: `${stats.videos.active} actifs`,         color: 'text-pink-500',   bg: 'bg-pink-50',    link: '/videos'     },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500">Bienvenue sur votre espace d'administration Borgel &amp; Associés</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        {cards.map(({ icon: Icon, label, value, sub, color, bg, link }, i) => (
          <motion.div key={label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
            <Link to={link} className="stat-card flex flex-col gap-3 hover:border-[#00d4b8] transition block group">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link
              ...
              style={{ color: "#00b8a0" }}
              onMouseEnter={(e) => (e.currentTarget.style.color = "#007a6b")}
              onMouseLeave={(e) => (e.currentTarget.style.color = "#00b8a0")}
            >
                ...
                >
                  <div
                    className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                      !c.is_read ? 'text-[#00b8a0]' : 'bg-slate-100 text-slate-500'
                    }`}
                    style={!c.is_read ? { background: "#ccfbf1" } : {}}
                  >
                    {c.name[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium text-slate-800 truncate group-hover:text-[#00b8a0] transition">{c.name}</p>
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    {!c.is_read && <div className="w-1.5 h-1.5 rounded-full" style={{ background: "#00d4b8" }} />}
                  </div>
                </Link>
              )
            )}
        ...
        <div className="glass-card p-5">
          ...
          <div className="space-y-1">
            {quickLinks.map(({ icon: Icon, label, path, desc }) => (
              <Link
                ...
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition group"
              >
                <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:bg-[#f0fdfa] transition">
                  <Icon size={14} className="text-slate-400 group-hover:text-[#00b8a0] transition" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-700 group-hover:text-[#00b8a0] transition">{label}</p>
                  <p className="text-xs text-slate-400">{desc}</p>
                </div>
                <ArrowRight size={13} className="text-slate-300 group-hover:text-[#00d4b8] transition" />
              </Link>
            ))}
          </div>
          ...
          <div className="mt-4 pt-4" style={{ borderTop: "1px solid #f1f5f9" }}>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">État du site</p>
            <div className="space-y-2">
              {[
                ...
                <div key={s.label} className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400">:{s.port}</span>
                </div>
              ))}
            </div>
          </div>
