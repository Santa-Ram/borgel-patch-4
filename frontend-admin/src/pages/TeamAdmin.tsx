...
import { Plus, Trash2, Search, CheckCircle, XCircle, Edit3, X, Camera, Upload, Phone, Mail, Link, ArrowLeft, Save } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import API from '../api/client';
import { showToast } from '../components/Alert';

const demoMembers = [
  { id:1, name:'Maître Borgel', role:'Avocat associé fondateur', biography:'Fondateur du cabinet en 2005, expert en dommage corporel.', is_active:true, order:1, phone:'0491335000', email:'borgel@cabinet.fr', linkedin:'', photo:null },
  { id:2, name:'Sophie Durand', role:'Avocate senior', biography:'Spécialisée en responsabilité médicale.', is_active:true, order:2, phone:'', email:'durand@cabinet.fr', linkedin:'', photo:null },
  { id:3, name:'Thomas Martin', role:'Avocat collaborateur', biography:'Collaborateur depuis 2020.', is_active:true, order:3, phone:'', email:'martin@cabinet.fr', linkedin:'', photo:null },
];
...
const emptyForm = {
  name:'', role:'', biography:'', phone:'', email:'',
...
export default function TeamAdmin() {
  ...
  useEffect(() => {
    Promise.all([
      API.get('/team/').then(r=>setMembers(r.data.results||r.data)).catch(()=>setMembers(demoMembers)),
  ...
  const handleSave = async () => {
    if(!form.name || !form.role) { showToast('error','Le nom et le rôle sont obligatoires'); return; }
    setSaving(true);
    try {
      let res;
      if(photoFile) {
      ...
      if(editing) {
        setMembers(p=>p.map(m=>m.id===editing.id?{...m,...res.data}:m));
  ...
  const toggleActive = async (m:any) => {
    try {
      ...
      setMembers(p=>p.map(x=>x.id===m.id?{...x,is_active:!x.is_active}:x));
  ...
  if(view==='form') return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">{editing?`Modifier ${editing.name}`:'Nouveau membre'}</h1>
          <p className="text-sm text-white/50">{editing?'Mettre à jour les informations':'Ajouter un membre à l\'équipe'}</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="glass-card p-5 flex flex-col items-center gap-4">
              ...
              <div className="w-32 h-32 rounded-2xl overflow-hidden bg-orange-500/10 border-2 border-dashed border-orange-500/30 flex items-center justify-center">
                {photoPreview ? (
                  ...
                  <div className="text-center">
                    <Camera size={28} className="text-orange-400/50 mx-auto mb-1"/>
                    <p className="text-[10px] text-white/30">Aucune photo</p>
                  </div>
                )}
              </div>
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-white">{form.name||'Nom du membre'}</p>
              <p className="text-xs text-white/50">{form.role||'Rôle'}</p>
            </div>
          </div>

          <div className="glass-card p-4 space-y-3">
            <p className="text-xs font-semibold text-white/60 uppercase tracking-wider">Statut</p>
            ...
            <div>
              ...
              <input type="number" value={form.order} onChange={e=>set('order',parseInt(e.target.value))}
                className="input-dark" min={0} max={99}/>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="glass-card p-5 space-y-4">
            <p className="text-xs font-semibold text-white/60 uppercase tracking-wider">Informations</p>
            <div>
              <label className="text-xs text-white/50 block mb-1.5">Nom complet *</label>
              <input type="text" value={form.name} onChange={e=>set('name',e.target.value)}
                className="input-dark" placeholder="Maître Borgel"/>
            </div>
            <div>
              <label className="text-xs text-white/50 block mb-1.5">Rôle *</label>
              ...
              {form.role==='custom'&&(
                <input type="text" placeholder="Rôle personnalisé..." className="input-dark mt-2"
                  onChange={e=>set('role',e.target.value)}/>
              )}
            </div>
            <div>
              <label className="text-xs text-white/50 block mb-1.5">Biographie</label>
              <textarea value={form.biography} onChange={e=>set('biography',e.target.value)}
                className="input-dark min-h-[120px] resize-none" rows={5}
                placeholder="Parcours, spécialisations, expériences..."/>
            </div>
          </div>

          <div className="glass-card p-5 space-y-3">
            <p className="text-xs font-semibold text-white/60 uppercase tracking-wider">Contact</p>
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-white/50 block mb-1.5 flex items-center gap-1"><Phone size={10}/>Téléphone</label>
                <input type="tel" value={form.phone} onChange={e=>set('phone',e.target.value)}
                  className="input-dark" placeholder="04 91 33 50 00"/>
              </div>
              <div>
                <label className="text-xs text-white/50 block mb-1.5 flex items-center gap-1"><Mail size={10}/>Email</label>
                <input type="email" value={form.email} onChange={e=>set('email',e.target.value)}
                  className="input-dark" placeholder="contact@borgel.fr"/>
              </div>
            </div>
          </div>

          <div className="glass-card p-5 space-y-3">
            <p className="text-xs font-semibold text-white/60 uppercase tracking-wider flex items-center gap-1"><Link size={11}/>Réseaux sociaux</p>
            {[
              {key:'linkedin', placeholder:'https://linkedin.com/in/...', label:'LinkedIn'},
              {key:'twitter',  placeholder:'https://twitter.com/...', label:'Twitter / X'},
          ...
          <div className="glass-card p-5">
            <p className="text-xs font-semibold text-white/60 uppercase tracking-wider mb-3">Expertises</p>
          ...
          <div className="flex gap-3">
            <button onClick={handleSave} disabled={saving} className="btn-primary flex-1 justify-center py-2.5">
              <Save size={15}/>
              {saving?'Sauvegarde...':(editing?'Mettre à jour':'Ajouter le membre')}
            </button>
            {editing && (
              <button onClick={()=>setConfirmDelete(editing)} className="btn-danger">
          ...
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{scale:0.9,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.9,opacity:0}}
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-white">Supprimer {confirmDelete.name} ?</h3>
                  <p className="text-sm text-white/60 mt-1">Cette action est irréversible. Le membre sera définitivement supprimé du site.</p>
                </div>
              </div>
              <div className="flex gap-3 justify-end">
                <button onClick={()=>setConfirmDelete(null)} className="btn-secondary">Annuler</button>
                <button onClick={()=>handleDelete(confirmDelete)} className="btn-danger"><Trash2 size={13}/>Supprimer</button>
              </div>
            </motion.div>
          </motion.div>
        )}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/50">{displayed.length} membre{displayed.length!==1?'s':''}</p>
        </div>
        <button onClick={openNew} className="btn-primary"><Plus size={15}/>Ajouter membre</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayed.map(m=>(
            <div key={m.id} className="glass-card p-4 hover:border-white/20 transition">
              <div className="flex items-start gap-3">
                <div className="w-12 h-12 rounded-xl overflow-hidden bg-orange-500/10 flex items-center justify-center flex-shrink-0">
                  {m.photo ? <img src={m.photo} alt={m.name} className="w-full h-full object-cover"/> : <span className="text-xl font-bold text-orange-400">{m.name[0]}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={()=>toggleActive(m)}>
                      {m.is_active
                        ? <span className="badge badge-green flex items-center gap-1"><CheckCircle size={9}/>Actif</span>
                        : <span className="badge badge-red flex items-center gap-1"><XCircle size={9}/>Inactif</span>}
                    </button>
                  </div>
                </div>
                <div className="flex flex-col gap-1 flex-shrink-0">
                  <button onClick={()=>openEdit(m)} className="btn-ghost p-1.5" title="Modifier"><Edit3 size={14}/></button>
                  <button onClick={()=>setConfirmDelete(m)} className="btn-ghost text-red-400 p-1.5" title="Supprimer"><Trash2 size={14}/></button>
                </div>
              </div>
            </div>
          ))}
          ...
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{scale:0.9}} animate={{scale:1}} exit={{scale:0.9}}
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-white">Supprimer {confirmDelete.name} ?</h3>
                  <p className="text-sm text-white/60 mt-1">Cette action est irréversible.</p>
                </div>
              </div>
              <div className="flex gap-3 justify-end">
                <button onClick={()=>setConfirmDelete(null)} className="btn-secondary">Annuler</button>
